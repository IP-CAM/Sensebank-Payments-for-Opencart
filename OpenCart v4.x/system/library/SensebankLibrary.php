<?php
namespace Opencart\System\Library\Extension\Sensebank;

define('SBPAYMENT_PAYMENT_NAME', 'Sense Bank');

define('SBPAYMENT_PROD_URL' , 'https://pay.sensebank.com.ua/ml/v1/payment/');
define('SBPAYMENT_TEST_URL' , 'https://sand.sensebank.com.ua/ml/v1/payment/');
...
class SensebankLibrary
{
    ...
    public function __construct()
    {
        ...
        if ($this->enable_cart_options === true){
            ...
            $this->discountHelper = new \Opencart\System\Library\Extension\Sensebank\SensebankDiscount();
		}
    ...
    {
        $file = DIR_EXTENSION . 'sensebank/library/' . str_replace('../', '', (string)$library) . '.php';
        if (file_exists($file)) {
            include_once($file);
        } else {
