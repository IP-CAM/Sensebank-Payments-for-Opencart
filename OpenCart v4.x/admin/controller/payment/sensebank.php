<?php
namespace Opencart\Admin\Controller\Extension\Sensebank\Payment;
...
class Sensebank extends \Opencart\System\Engine\Controller
{
    ...
    {
        //$method_library = $this->load->controller('extension/sensebank/common/sensebank_library');

		$this->library('SensebankLibrary');
		$method_library = new \Opencart\System\Library\Extension\Sensebank\SensebankLibrary($this->registry);
        ...
        array_push($data['breadcrumbs'],
            ...
            array(  // Payment by
                ...
                'href' => $this->url->link('extension/sensebank/payment/sensebank', 'user_token=' . $this->session->data['user_token'], 'SSL')
            )
        );

        $data['save'] = $this->url->link('extension/sensebank/payment/sensebank.save', 'user_token=' . $this->session->data['user_token']);
        ...
        $this->response->setOutput($this->load->view('extension/sensebank/payment/sensebank', $data));
    }
    ...
    public function save(): void {
        ...
        if (!$this->user->hasPermission('modify', 'extension/sensebank/payment/sensebank')) {
    ...
    public function _updateGateSettings($test_mode = true): void
    {
        $method = $this->load->controller('extension/sensebank/common/sensebank_library');

        $this->library('log');
        $file_name = "oc4x_sensebank_" . date("y-m") . ".log";
     ...
     * @return array
     */
    private function getTaxTypeList(): array
    {
        return [
            [
                'numeric' => 0,
     ...
     * @return array
     */
    private function getTaxSystemList(): array
    {
        return [
            [
                'numeric' => 0,
     ...
     * @return array
     */
    private function getFFDVersionlist(): array
    {
        return [
            [
                'value' => 'v1_05',
     ...
     * @return array
     */
    private function getPaymentMethodTypeList(): array
    {
        return [
            [
                'numeric' => 1,
     ...
     * @return array
     */
    private function getPaymentObjectTypeList(): array
    {
        return [
            [
                'numeric' => 1,
    ...
    private function library($library): void
    {
    ...
    public function install(): void
    {
        ...
        $this->model_extension_sensebank_payment_sensebank->install();
    }
    public function uninstall(): void
    {
        $this->load->model('extension/sensebank/payment/sensebank');
        $this->model_extension_sensebank_payment_sensebank->deleteTables();
    }
    public function order()
    {
        $this->language->load('extension/sensebank/payment/sensebank');
        $this->load->model('extension/sensebank/payment/sensebank');
        $this->load->model('sale/order');
        $data['gateway_order'] = $this->model_extension_sensebank_payment_sensebank->getGatewayOrder($this->request->get['order_id']);
        $order_info = $this->model_sale_order->getOrder($this->request->get['order_id']);
        //$this->load->library('Sensebank');
		$method_library = $this->load->controller('extension/sensebank/common/sensebank_library');

        if (isset($method_library->enable_refund_options) && $method_library->enable_refund_options === true &&
            ...
            return $this->load->view('extension/sensebank/payment/sensebank_order', $data);
        }
    }
    public function gatewayOrderAction()
    {
        ...
        //$this->load->library('Sensebank');
		$method_library = $this->load->controller('extension/sensebank/common/sensebank_library');
        $gateway_order = $this->model_extension_sensebank_payment_sensebank->getGatewayOrder($this->request->get['order_id']);
        $order_info = $this->model_sale_order->getOrder($this->request->get['order_id']);
        ...
        if ($gateway_order && $order_info && !empty($this->request->post['order_action'])) {
            ...
            if ($order_action == 'payment_status') {
                $response = $method_library->_sendGatewayData($parameters, $action_adr . "getOrderStatusExtended.do");
                ...
                if (!empty($response) && isset($response['errorCode'])) {
                    if (empty($response['errorCode'])) {
                        ...
                        $this->model_extension_sensebank_payment_sensebank->updateGatewayOrder($order_info['order_id'], $sql_data);
                        $json['history'] = array(
