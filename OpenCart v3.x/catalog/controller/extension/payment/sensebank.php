...
class ControllerExtensionPaymentSensebank extends Controller
{
    ...
    public function __construct($registry)
    {
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/sensebank.twig')) {
    ...
    public function payment()
    {
        ...
        if (isset($response['orderId'])) {
            ...
            $this->model_checkout_order->addOrderHistory($order_number, $this->config->get('payment_sensebank_order_status_before_id'), $comment, false);
        }
    ...
    private function initializeGatewayLibrary()
    {
        ...
        $this->method_library = new Sensebank();
        ...
        if (file_exists(DIR_SYSTEM . "library/sensebank_cacert.cer") && $this->config->get('payment_sensebank_enable_sensebank_cacert') == true) {
            $this->method_library->enable_sensebank_cacert = $this->config->get('payment_sensebank_enable_sensebank_cacert');
            $this->method_library->sensebank_cacert_path = DIR_SYSTEM . "library/sensebank_cacert.cer";
        } else {
    ...
    public function _storeGatewayOrderData($orderId, $order_id, $response)
    {
        ...
        $this->model_extension_payment_sensebank->storeGatewayOrder($data);
    }

    public function _getGatewayOrderByReference($orderId)
    {
        $this->load->model('extension/payment/sensebank');
        return $this->model_extension_payment_sensebank->getGatewayOrderByReference($orderId);
    }

    public function _updateGatewayOrder($order_id, $data)
    {
        $this->load->model('extension/payment/sensebank');
        $this->model_extension_payment_sensebank->updateGatewayOrder($order_id, $data);
    }
}